...
Widget BuildOrderStatus(double screenHeight,double screenWidth,String title,{bool fromOrderDetail=false,bool fromDeliveryMan=false}) {
  return Container(
    ...
    decoration: BoxDecoration(
      color: title == 'Delivered' ? AppColors.greenLight :title=='Pending'?AppColors.orangeLight :title =='In Progress'||title=='one Time'?
      AppColors.secondaryColorWithOpacity16:title=='Coupon Based'?AppColors.secondaryColorWithOpacity8:AppColors.redLight,
      borderRadius: BorderRadius.circular(8),//kl
    ),
    child: Center(
      child: Text(
        title,
        style: TextStyle(
          ...
          title=='Pending'?AppColors.orangeColor: title =='In Progress'||title=='one Time'?AppColors.secondary:title=='Coupon Based'?AppColors.secondary:
          AppColors
              .redColor,
